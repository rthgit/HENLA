...
class HENLA0:
    ...
    def __init__(
        ...
        self.category_tracker = CategoryTracker(self.concept_tracker)
        self.language_grounder = LanguageGrounder(self.concept_tracker, self.category_tracker)
        self.text_reader = TextReader()
        self.micro_signal_extractor = MicroSignalExtractor()
        self.micro_events: list[dict] = []
        self.scratchpad_manager = ScratchpadManager()
        self.last_scratchpad: dict | None = None
        self.scratchpad_history: list[dict] = []
        self.deliberation_events: list[dict] = []
        self.subgraph_registry = SubgraphRegistry()
        self.cognitive_areas = CognitiveAreaSystem(self.subgraph_registry)
        self.cognitive_areas.initialize_default_areas()
    ...
    def step(self, action_type: str, target: str,
        ...
        scratchpad = self.scratchpad_manager.open(
            self.state.to_dict(),
            active_question=f"Should HENLA execute {action_type} on {target}?",
        )
        self.scratchpad_manager.activate(scratchpad, activated_nodes, self.graph)
        ...
        self.scratchpad_manager.add_hypothesis(
            scratchpad,
            claim=f"{action_type} may produce {predicted_result}",
            confidence=confidence,
            source=["hypergraph_prediction"],
            predicted_delta_viability=pred_valence,
        )
        self.scratchpad_manager.simulate_candidates(
            scratchpad,
            self.graph,
            [(action_type, target)],
        )
        self.scratchpad_manager.select_action(scratchpad, action_type, target)
        ...
        energy_spent = min(1.0, energy_spent + parameters.get("_deliberation_cost", 0.0))
        ...
        micro_event = self.micro_signal_extractor.extract_episode(ep)
        self.micro_events.append(micro_event)
        self.micro_events = self.micro_events[-20:]
        self.scratchpad_manager.reflect(
            scratchpad,
            observed_result=result_status,
            observed_valence=vr.valence,
            prediction_error=pe,
        )
        self.scratchpad_manager.close(scratchpad)
        self.last_scratchpad = scratchpad.to_dict()
        self.scratchpad_history.append(self.last_scratchpad)
        self.scratchpad_history = self.scratchpad_history[-10:]
        if is_repeated_failure:
            self.loop_events.append({
                ...
                "loop_penalty": vr.components.get("loop_penalty", 0.0),
            })
        ...
        if self.episode_store_path:
            self.episode_store.append(self.episode_store_path, ep)
    ...
    def _sense(self, modality: str, target: str) -> dict:
        if modality == "filesystem":
            ...
            return self.language_grounder.process_text(self.graph, target)
        elif modality == "reading":
            return self.text_reader.read_text(self.graph, target)
    ...
    def _act(self, action_type: str, target: str,
        ...
        elif action_type == "sense_text":
            output = self.language_grounder.process_text(self.graph, target)
            ok = output["success"]

        elif action_type == "read_text":
            source = parameters.get("source", "inline")
            output = self.text_reader.read_text(self.graph, target, source=source)
            ok = output["success"]
    ...
    def report(self) -> dict:
        concepts = self.concept_tracker.evaluate_graph(self.graph)
        categories = self.category_tracker.evaluate_graph(self.graph)
        return {
            "cycle": self.cycle,
            ...
            "loops": {
                "total": len(self.loop_events),
                "recent": self.loop_events[-10:],
            },
            "episode_count": len(self.episodes),
            "scratchpad": {
                "last": self.last_scratchpad,
                "recent_count": len(self.scratchpad_history),
            },
            "deliberation": {
                "total": len(self.deliberation_events),
                "recent": self.deliberation_events[-10:],
            },
            "micro_signals": {
                "total": len(self.micro_events),
                "recent": self.micro_events[-10:],
            },
            "subgraphs": self.subgraph_registry.to_dict(),
            "cognitive_areas": self.cognitive_areas.to_dict(),
            "episode_store": self.episode_store.summarize(self.episode_store_path).to_dict()
            if self.episode_store_path else None,
            "mean_valence": round(
...
class HENLA0Autonomous(HENLA0):
    ...
    def autonomous_step(self, modality: str = "filesystem",
        ...
        self.selector.register_outcome(best[0], best[1], ep.valence, ep.result.status)
    ...
    def deliberative_step(
        ...
        """
        Select an action by simulating multiple selector candidates first.
        The scratchpad remains temporary; only summary metrics are retained.
        """
        self.discover_targets()
        candidates = self.selector.select(
            state=self.state,
            known_targets=self._known_targets,
            modality=modality,
            top_k=max(2, candidate_count),
            temperature=temperature,
        )
        if not candidates:
            return self.autonomous_step(modality=modality, temperature=temperature)

        scratchpad = self.scratchpad_manager.open(
            self.state.to_dict(),
            active_question="Which candidate has the best expected viability after simulation?",
        )
        candidate_pairs = [(candidate.action_type, candidate.target) for candidate in candidates]
        self.scratchpad_manager.activate(
            scratchpad,
            [item for pair in candidate_pairs for item in pair],
            self.graph,
        )
        simulations = self.scratchpad_manager.simulate_candidates(
            scratchpad,
            self.graph,
            candidate_pairs,
        )
        viable = [simulation for simulation in simulations if simulation["decision"] != "reject"]
        pool = viable if viable else simulations
        best_simulation = max(
            pool,
            key=lambda item: (item["predicted_valence"], item["confidence"], -item["risk"]),
        )
        rejected = [
            simulation for simulation in simulations
            if simulation is not best_simulation
        ]
        scratchpad.rejected_actions = rejected[: self.scratchpad_manager.max_notes]
        self.scratchpad_manager.add_hypothesis(
            scratchpad,
            claim=(
                f"{best_simulation['candidate_action']} on {best_simulation['target']} "
                "has best simulated viability"
            ),
            confidence=best_simulation["confidence"],
            source=["deliberative_selector", "reasoner"],
            predicted_delta_viability=best_simulation["predicted_valence"],
        )
        self.scratchpad_manager.select_action(
            scratchpad,
            best_simulation["candidate_action"],
            best_simulation["target"],
        )

        deliberation_cost = min(0.20, 0.01 * len(simulations))
        ep = self.step(
            best_simulation["candidate_action"],
            best_simulation["target"],
            {"_deliberation_cost": deliberation_cost},
            modality,
        )
        self.scratchpad_manager.reflect(
            scratchpad,
            observed_result=ep.result.status,
            observed_valence=ep.valence,
        ...
        self.scratchpad_history.append(self.last_scratchpad)
        self.scratchpad_history = self.scratchpad_history[-10:]
        simulation_error = abs(best_simulation["predicted_valence"] - ep.valence)
        action_regret = max(
            [simulation["predicted_valence"] for simulation in simulations]
            or [best_simulation["predicted_valence"]]
        ) - best_simulation["predicted_valence"]
        event = {
            "cycle": self.cycle,
            "candidate_count": len(simulations),
            "selected_action": best_simulation["candidate_action"],
            "selected_target": best_simulation["target"],
            "predicted_valence": round(best_simulation["predicted_valence"], 4),
            "observed_valence": ep.valence,
            "simulation_error": round(simulation_error, 4),
            "action_regret": round(action_regret, 4),
            "avoided_negative_actions": len([
                simulation for simulation in simulations
                if simulation["decision"] == "reject"
            ]),
            "deliberation_cost": round(deliberation_cost, 4),
            "scratchpad_id": scratchpad.scratchpad_id,
        }
        self.deliberation_events.append(event)
        self.deliberation_events = self.deliberation_events[-20:]
        self.selector.register_outcome(
            best_simulation["candidate_action"],
            best_simulation["target"],
            ep.valence,
            ep.result.status,
        )
        if best_simulation["candidate_action"] == "list_dir":
            self.discover_targets()
        return ep
